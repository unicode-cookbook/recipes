...
compare <- function(jipa, profile) {
	text_in <- paste0("sources/", jipa, "_input.txt")
	text_out <- paste0("sources/", jipa, "_output.txt")
...
profile <- "data/validIPA2005_graphemes.tsv"
...
profile <- "data/strictIPA2005_graphemes.tsv"
compare("Brazilian_Portuguese", profile)
compare("Kabiye", profile)
compare("Zurich_German", profile)
compare("Vietnamese", profile)
...
tailored <- function(jipa) {
	input <- scan(paste0("sources/", jipa, "_input.txt")
				, what = "character"
				, sep = "\n"
				)
